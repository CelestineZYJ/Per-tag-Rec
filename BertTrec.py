...
def average_user_tweet(user, train_df, con_emb_dict):
    ...
    for content in train_df['content'].loc[train_df['user_id'] == user]:
...
def average_hashtag_tweet(hashtag, train_df, con_emb_dict):
    ...
    # train_df.to_csv(tagEeb, sep='\t', index=False)
    for content in train_df['content'].loc[train_df['hashtag'] == hashtag]:
...
def rank_hashtag(train, content_emb):
    ...
    cosine_similar('TalkingTeenage', '#teens', train_df, con_emb_dict)
